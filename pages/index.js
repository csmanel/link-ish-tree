    ...
    >
      <Head>
        ...
        <link
          rel="icon"
          href={
            process.env.NEXT_PUBLIC_CLOUDIMG_TOKEN
              ? `https://${process.env.NEXT_PUBLIC_CLOUDIMG_TOKEN}.cloudimg.io/${config.avatar}?radius=500`
              : `${config.avatar}`
          }
        />
      </Head>
      <div className="flex flex-col justify-center align-center w-full lg:w-1/2 md:w-1/3 h-[80%] lg:h-1/2 md:h-1/2 items-center">
        <div className="w-full h-full flex flex-col justify-center items-center">
          ...
          <h1
            className="text-[${config.textColor}] text-center text-md font-normal mt-5"
            style={{ color: `${config.textColor}` }}
          >
            {config.description}
          </h1>
